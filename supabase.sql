-- TAMA Store Supabase setup
-- 1) Create Auth user in Authentication > Users with the admin email/password.
-- 2) Run this SQL in SQL Editor.
-- 3) The INSERT at the bottom automatically adds that Auth user to admins.

create extension if not exists pgcrypto;

drop function if exists public.get_or_create_chat(text,text,text);
drop function if exists public.get_user_chat(uuid,text);
drop function if exists public.send_user_chat_message(uuid,text,text);
drop function if exists public.close_user_chat(uuid,text);

table if not exists public.admins (
  user_id uuid primary key references auth.users(id) on delete cascade,
  created_at timestamptz not null default now()
);

table if not exists public.products (
  id uuid primary key default gen_random_uuid(),
  title text not null,
  description text not null,
  price integer not null check (price >= 20000 and price <= 1000000),
  budget text not null check (budget in ('20-100','200-1000')),
  thumbnail_url text not null,
  thumbnail_path text,
  created_at timestamptz not null default now()
);

-- Compatibility with earlier versions of this project.
...
-- Optional legacy column requested by older code; it is NOT used by this version.
...
-- User-side RPCs keep anonymous chat data behind the access token.
create or replace function public.get_or_create_chat(p_username text, p_category text, p_token text)
...
begin
  ...
  if c.id is null then
    insert into public.chats(username,category,access_token) values(p_username,p_category,p_token) returning * into c;
  end if;
  return c;
end;
$$;
...
begin
  ...
  insert into public.chat_messages(chat_id,sender_type,message) values(p_chat_id,'user',p_message) returning * into m;
...
-- Add your Auth user to the admin table automatically.
insert into public.admins(user_id)
select id from auth.users where lower(email)=lower('fyesty8@gmail.com')
on conflict (user_id) do nothing;

-- Budget constraint: selected category must match nominal.
